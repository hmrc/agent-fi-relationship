...
class RelationshipControllerFlagOnSpec
    extends UnitSpec
    with MockitoSugar
  ...
  "RelationshipController (Both Flags On)" should {
    ...
    "return Status: NOT_FOUND if any previous relationships are found" in {
      ...
      verify(mockMongoService, times(1)).findRelationships(any(),
                                                           ...
                                                           any(),
                                                           any())(any())
      verify(mockMongoService, times(1)).findAnyRelationships(any(),
                                                              any(),
                                                              any())(any())
    }
    ...
    "return Status: NOT FOUND when no relationship in Cesa and Agent Mapping is found" in {
      ...
      when(
        mockCesaRelationship
          .lookupCesaForOldRelationship(
            eqs(Arn(validTestArn)),
            eqs(Nino(validTestNINO)))(any(), any(), any(), any()))
