...
import uk.gov.hmrc.agentfirelationship.services.{
  CesaRelationshipCopyService,
  RelationshipMongoService
}
...
class RelationshipControllerSpec
    extends UnitSpec
    with MockitoSugar
  ...
  val controller = new RelationshipController(mockAuditService,
                                              mockMongoService,
  ...
  "RelationshipController" should {
    "return Status: OK when successfully finding a relationship" in {
      when(
        mockMongoService.findRelationships(eqs(validTestArn),
                                           eqs(testService),
                                           eqs(validTestNINO),
                                           eqs(Active))(any()))
        .thenReturn(Future successful List(validTestRelationship))

      val response = controller.findRelationship(validTestArn,
                                                 testService,
                                                 validTestNINO)(fakeRequest)
    ...
    "return Status: NOT_FOUND for not finding data" in {
      ...
      when(
        mockMongoService.findAnyRelationships(eqs(validTestArn),
                                              eqs(testService),
                                              eqs(validTestNINO))(any()))
        .thenReturn(Future successful List())
    ...
    "return Status: CREATED for creating new record as a client" in {
      ...
      when(
        mockAuditService
          .sendCreateRelationshipEvent(any())(any(), any(), any()))
      ...
      val response =
        controller.createRelationship(validTestArn, testService, validTestNINO)(
          fakeCreateRequest)
    ...
    "send an audit event if the relationship is successfully created" in {
      ...
      when(
        mockMongoService.createRelationship(eqs(validTestRelationship))(any()))
        .thenReturn(Future successful (()))
      when(
        mockMongoService.findRelationships(eqs(validTestArn),
                                           eqs(testService),
    ...
    "return Status: FORBIDDEN when logged in Agent ARN does not match given ARN when creating relationship" in {
      ...
      val response =
        controller.createRelationship("JARN0000001",
                                      testService,
                                      validTestNINO)(fakeCreateRequest)

      status(response) shouldBe FORBIDDEN
      verify(mockMongoService, times(0)).createRelationship(any())(any())
      verify(mockMongoService, times(0)).findRelationships(any(),
                                                           any(),
                                                           any(),
                                                           any())(any())
    }
    ...
    "return Status: FORBIDDEN when logged in Client NINO does not match given NINO when creating relationship" in {
      ...
      val response =
        controller.createRelationship(validTestArn, testService, "AB123456C")(
          fakeCreateRequest)
    ...
    "return Status: OK for deleting a record as a client" in {
      authStub(clientAffinityAndEnrolments)
      when(
        mockMongoService.terminateRelationship(eqs(validTestArn),
                                               eqs(testService),
                                               eqs(validTestNINO))(any()))
      ...
      when(
        mockAuditService
          .sendTerminatedRelationshipEvent(any())(any(), any(), any()))
        .thenReturn(Future successful (()))

      val response =
        controller.terminateRelationship(validTestArn,
                                         testService,
                                         validTestNINO)(fakeRequest)

      status(response) shouldBe OK
      verify(mockMongoService, times(1)).terminateRelationship(any(),
                                                               any(),
                                                               any())(any())
    }
    ...
    "return Status: OK for deleting a record as hmrc" in {
      ...
      when(
        mockAuditService
          .sendHmrcLedDeleteRelationshipAuditEvent(any())(any(), any(), any()))
        .thenReturn(Future successful (()))
    ...
    "return Status: FORBIDDEN when logged in Agent ARN does not match given ARN when deleting relationship" in {
      ...
      val response =
        controller.terminateRelationship("JARN0000001",
                                         testService,
                                         validTestNINO)(fakeRequest)

      status(response) shouldBe FORBIDDEN
      verify(mockMongoService, times(0)).terminateRelationship(any(),
                                                               any(),
                                                               any())(any())
    }
    ...
    "return Status: FORBIDDEN when logged in Client NINO does not match given NINO when deleting relationship" in {
      ...
      val response = controller.terminateRelationship(validTestArn,
                                                      testService,
                                                      "AB123456C")(fakeRequest)
    ...
    "return Status: OK for finding data via access control endpoint" in {
      when(
        mockMongoService
          .findRelationships(eqs(validTestArn),
                             eqs(testService),
                             eqs(validTestNINO),
                             eqs(RelationshipStatus.Active))(any()))
        .thenReturn(Future successful List(validTestRelationship))
    ...
    "return Status: OK with json body of all agent invitations with TERMINATED status" in {
      ...
      authStub(agentAffinityAndEnrolmentsCreds)
      when(
        mockAgentClientAuthConnector.authorisedForAfi(strideRole)(agentAction))
        .thenReturn(Future successful Ok)
      when(
        mockMongoService.findInactiveAgentRelationships(eqs(validTestArn))(
          any()))
        .thenReturn(Future successful List(validTestRelationshipTerminated,
                                           validTestRelationshipCesa))
    ...
    "return Status: OK with json body of all agent invitations with ACTIVE status" in {
      ...
      when(
        ...
        .thenReturn(Future successful Ok)
      when(
        mockMongoService.findActiveAgentRelationships(eqs(validTestArn))(any()))
        .thenReturn(Future successful List(validTestRelationshipTerminated,
                                           validTestRelationshipCesa))
    ...
    "return Status: OK with json body of all client invitations with TERMINATED status" in {
      ...
      when(mockAgentClientAuthConnector.authorisedForAfi(strideRole)(action))
        .thenReturn(Future successful Ok)
      when(
        mockMongoService.findInactiveClientRelationships(eqs(validTestNINO))(
          any()))
        .thenReturn(Future successful List(validTestRelationshipTerminated,
                                           validTestRelationshipCesa))
    ...
    "return Status: OK with json body of all client invitations with ACTIVE status" in {
      ...
      when(mockAgentClientAuthConnector.authorisedForAfi(strideRole)(action))
        .thenReturn(Future successful Ok)
      when(
        mockMongoService.findActiveClientRelationships(eqs(validTestNINO))(
          any()))
        .thenReturn(Future successful List(validTestRelationshipTerminated,
                                           validTestRelationshipCesa))
