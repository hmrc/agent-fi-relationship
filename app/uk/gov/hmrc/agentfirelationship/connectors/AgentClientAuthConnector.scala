...
class AgentClientAuthConnector @Inject() (val authConnector: AuthConnector)(using ec: ExecutionContext)
  ...
  def authorisedForAfi(strideRoles: Seq[String])(action: AfiAction)(using hc: HeaderCarrier): Future[Result] =
    authorised()
  ...
  def onlyStride(
      strideRole: String
  )(action: => Future[Result])(using hc: HeaderCarrier, ec: ExecutionContext): Future[Result] =
