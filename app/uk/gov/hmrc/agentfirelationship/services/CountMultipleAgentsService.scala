...
@Singleton
class CountMultipleAgentsService @Inject() (
  ...
  if (enabled) {
    ...
    lockService
      .withLock {
        ...
        for {
          queryResult <- relationshipMongoRepository.countMultipleAgents()
          _ = logger.warn(
            s"Count of multiple agents per client id: ${queryResult.map(_.toBsonDocument()).toString()}"
          )
        } yield ()
      }
